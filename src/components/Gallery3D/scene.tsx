...
function MediaPlane({
  ...
  useFrame(() => {
    ...
    const nearFade =
      absDepth >= NEAR_FADE_START
        ? 1
        : absDepth <= NEAR_FADE_END
        ? 0
        : (absDepth - NEAR_FADE_END) / Math.max(NEAR_FADE_START - NEAR_FADE_END, 0.0001);

    const target = Math.min(gridFade, nearFade);
      ...
      <div className={styles.container}>
        ...
        {showControls && (
          <div className={styles.controlsPanel}>
            {isTouchDevice ? (
              <>
                <b>Glisser</b> Déplacer · <b>Pincer</b> Zoom
              </>
            ) : (
              <>
                <b>ZQSD</b> Déplacer · <b>AE</b> Haut/Bas · <b>Scroll</b> Zoom
              </>
            )}
          </div>
        )}
